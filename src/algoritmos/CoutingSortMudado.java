...
public class CountingSort {
    ...
    private static int findMaximumNumberCasos(Vector<CovidData>  vector) {
        ...
        for (int i = 0; i < vector.size(); i++) {
            if(vector.findWithIndex(i).getAvailableConfirmed() > numberMaximum) numberMaximum = vector.findWithIndex(i).getAvailableConfirmed(); //numeros de casos confirmados
        }
    ...
    private static int findMaximumNumberObitos(Vector<CovidData> vector) {
        int numberMaximum = 0;
        for (int i = 0; i < vector.size(); i++) {
            if(vector.findWithIndex(i).getAvailableDeaths() > numberMaximum) numberMaximum = vector.findWithIndex(i).getAvailableDeaths(); //numero de obitos confirmados
        }
        return numberMaximum;
    }
}
